/* A small, self-contained birthday story. No analytics, remote fonts or CDNs.
...
function setup(title, sub, theme = "A LITTLE BIRTHDAY MAGIC", dark = false) {
  clean();
...
function opening(gate = false) {
  ...
  if (gate) {
    app.innerHTML = `<section class="gate"><div class="eyebrow">A GIFT FROM MY HEART TO YOURS</div>${lamp(true)}<h1>A little light.<br>A lot of love.</h1><p>Some birthdays are celebrated…<br>Some birthdays become memories.</p>${button("enter", "Tap to begin, Kudu ♡")}</section>`;
    on($("#enter"), "click", () => {
      bell();
      opening();
    });
...
const scenes = [
  ...
  function findKudu() {
    ...
    setup(
      "Where is my Kudu?",
      "Find the portrait of just you, birthday girl.",
      "MY FAVOURITE FACE",
    );
    $("#stage").innerHTML =
  ...
  function sadhya() {
    const msg = "Kudu’s birthday Sadhya is ready! 😋 ♡";
    setup(
      "A birthday, Kerala style.",
    ...
    $("#stage").innerHTML =
      '<div class="banana-leaf" aria-label="Birthday sadhya"></div><div class="dish-options">' +
      dishes
  ...
  function payasam() {
    const msg =
      "Birthday calories don’t count. Especially payasam. ഒരു ഗ്ലാസ് കൂടി വേണോ Kudu? ♡";
    setup(
  ...
  function pookalam() {
    ...
    setup(
      ...
      "Tap around the circle to make our birthday Pookalam.",
  ...
  function family() {
    ...
    const keys = ["familyA", "familyB", "wedding", "car"];
    $("#stage").innerHTML =
    ...
    $$("[data-photo]", $("#stage")).forEach((b) =>
      on(b, "click", () => {
        ...
        if (seen.size === 4) done(msg);
      }),
    );
    hint(
      "Tap each of the four photographs, then use × to close the full-screen view.",
    );
    restoreDone(msg);
  },
  function letter() {
  ...
  function music() {
    ...
    if (KUDU.music) {
      ...
      on($("#play"), "click", async () => {
        try {
          ...
          $("#music-note").textContent =
            "The song is resting for now. We can share the quiet instead.";
        }
      });
      on(audio, "timeupdate", () => {
        if (audio.duration) {
      ...
      on(audio, "error", () => {
        $("#music-note").textContent =
          "This song could not be loaded. Our quiet moment is still here.";
      });
    } else {
      $("#play").disabled = true;
    ...
    hint(
      "Enjoy a quiet moment and tap “Stay in this quiet moment.” If a song has been added, you can play it here.",
    );
    restoreDone(msg);
  },
  function tea() {
  ...
  function decorate() {
    const msg = "Perfect. Almost as cute as the birthday girl.";
    setup(
  ...
  function candles() {
    ...
    setup(
      ...
      "Press and hold the button to send a little birthday breeze.",
      "A WISH JUST FOR YOU",
    );
    $("#stage").innerHTML =
  ...
  function cutCake() {
    const msg = "Happy Birthday Kudu ♡ First piece for me, obviously.";
    setup(
...
function cake(classes = "") {
  return `<div class="cake-scene ${classes}" aria-label="Kudu’s birthday cake"><div class="cake-plate"></div><div class="cake-body"></div><div class="cake-top"></div><div class="cake-flowers">✿ ✿</div><div class="cake-topper">Kudu</div><div class="cake-heart">♡</div><div class="candles"><i class="candle"></i><i class="candle"></i><i class="candle"></i></div><div class="cake-slice"></div></div>`;
}
function finale() {
  ...
  app.innerHTML = `<section class="final"><div class="eyebrow" style="justify-content:center">YOU ARE SO LOVED</div><div class="three-hearts" aria-hidden="true"><span>♡</span><span>♡</span><span>♡</span></div><article class="final-letter"><h2>Happy Birthday<br>Kudu ♡</h2>This birthday, I celebrate you.

The woman I love.
My wife. My home.
And now, the beautiful mother of our little one.

This year we celebrate your birthday together…

but next birthday,
there will be a tiny pair of hands trying to grab your cake,
a little person probably stealing all the attention,
and our little Junior celebrating Amma’s birthday with us. ♡

So enjoy this birthday, Kudu.
